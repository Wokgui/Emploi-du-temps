        ...
        return """
            (function(){
              try{
                ...
                function datedDay757(d){
                  const language=language757(),weekday=weekday757(d),longDate=long757(d);
                  if(language==='fr')return weekday+' le '+(d.getDate()===1?'1er '+d.toLocaleDateString(locale757(),{month:'long',year:'numeric'}):longDate);
                  if(language==='de')return weekday+', '+longDate;
                ...
                function todayHead757(date,letter){ensureToday757();const h=document.getElementById('todayTitle'),d=document.getElementById('todayDate');if(h)h.textContent=datedDay757(date);if(d)d.textContent=tr757('Semaine ','Week ','Woche ')+letter}
