        ...
        return """
            (function(){
              try{
                ...
                function settingsHost(){const modal=document.getElementById('settingsModal');if(!modal)return null;return modal.querySelector('.modalContent,.modalBody,.settingsContent,.settingsPanel,.settings-scroll')||modal.querySelector('[class*=content]')||modal}
                function installSettings(){const host=settingsHost();if(!host||document.getElementById('week658Settings'))return;const s=load(),box=document.createElement('section');box.id='week658Settings';box.innerHTML='<div class="w658Title">'+tr('Couleurs de la vue semaine','Week view colours','Farben der Wochenansicht')+'</div><div class="w658Colors"><span>'+tr('Cases libres','Free cells','Freie Felder')+'</span><input id="w658Free" type="color"><span>'+tr('Cours','Classes','Unterricht')+'</span><input id="w658Course" type="color"><span>'+tr('Midi','Lunch','Mittag')+'</span><input id="w658Lunch" type="color"></div><div class="w658Days"><div class="w658Title">'+tr('Midi par jour','Lunch by day','Mittag pro Tag')+'</div></div>';host.appendChild(box);box.querySelector('#w658Free').value=s.free;box.querySelector('#w658Course').value=s.course;box.querySelector('#w658Lunch').value=s.lunch;[['w658Free','free'],['w658Course','course'],['w658Lunch','lunch']].forEach(([id,k])=>box.querySelector('#'+id).addEventListener('input',e=>{const x=load();x[k]=e.target.value;save(x)}));const days=box.querySelector('.w658Days');[2,3,4,5,6,7,1].forEach(d=>{if(!document.documentElement.classList.contains('weekendScheduleEnabled')&&(d===7||d===1))return;const cfg=s.days[d]||{enabled:true,start:720},row=document.createElement('div');row.className='w658Day';row.innerHTML='<b>'+DAYN[d]+'</b><label><input type="checkbox" '+(cfg.enabled===false?'':'checked')+'> '+tr('Afficher Midi','Show lunch','Mittag zeigen')+'</label><select><option value="720">12 h – 13 h</option><option value="780">13 h – 14 h</option></select>';row.querySelector('select').value=String(cfg.start||720);row.querySelector('input').onchange=e=>{const x=load();x.days[d]=Object.assign({},x.days[d]||{start:720},{enabled:e.target.checked});save(x)};row.querySelector('select').onchange=e=>{const x=load();x.days[d]=Object.assign({},x.days[d]||{enabled:true},{start:+e.target.value});save(x)};days.appendChild(row)})}
