        ...
        return """
            (function(){
              try{
                ...
                let queued=false,migrated=false;
                ...
                function save(settings){settings=normalize(settings);try{localStorage.setItem(KEY,JSON.stringify(settings))}catch(e){}persistNative(settings);schedule()}
                ...
                function installSettings(){
                  ...
                  const days=box.querySelector('.w658Days');wantedDays.forEach(day=>{
                    ...
                    row.innerHTML='<b>'+DAYN[day]+'</b><label><input class="w658Enabled" type="checkbox" '+(cfg.enabled===false?'':'checked')+'><span>'+tr('Midi','Lunch','Mittag')+'</span></label><span class="timeShell740"><input class="w658Start" type="time" step="300" value="'+clock(cfg.start)+'"><span class="timeValue740">'+clock(cfg.start)+'</span></span><span class="timeShell740"><input class="w658End" type="time" step="300" value="'+clock(cfg.end)+'"><span class="timeValue740">'+clock(cfg.end)+'</span></span>';
                    const enabled=row.querySelector('.w658Enabled'),start=row.querySelector('.w658Start'),end=row.querySelector('.w658End');
                    const syncTime=()=>{const sv=start.parentElement&&start.parentElement.querySelector('.timeValue740'),ev=end.parentElement&&end.parentElement.querySelector('.timeValue740');if(sv)sv.textContent=start.value||'--:--';if(ev)ev.textContent=end.value||'--:--'};
                    const commit=()=>{const value=load(),startValue=toMin(start.value);let endValue=toMin(end.value);if(startValue<0)return;if(endValue<=startValue){endValue=Math.min(1440,startValue+60);end.value=clock(endValue)}syncTime();value.days[day]={enabled:enabled.checked,start:startValue,end:endValue};save(value)};
