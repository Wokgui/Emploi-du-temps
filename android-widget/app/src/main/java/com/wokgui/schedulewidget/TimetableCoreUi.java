        ...
        return """
            (function(){
              try{
                ...
                function firstFree(root,w,d){ensureWeek(root,w);const used=new Set(root._weeks[w][String(d)].courses.map(c=>Number(c.slot)).filter(Boolean)),available=slots(root);for(let i=0;i<available.length;i++){const n=Number(available[i].n)||i+1;if(!used.has(n))return n}return Number(available[0]?.n)||1}
                ...
                function style(){if(document.getElementById('bulkFixedStyle'))return;const s=document.createElement('style');s.id='bulkFixedStyle';s.textContent=`
                  #addBulkCourses{display:block!important}
                ...
                function slotOpts(root,v){return slots(root).map((s,i)=>{const n=Number(s.n)||i+1;return `<option value="${n}" ${n===Number(v)?'selected':''}>${n}e · ${s.start}–${s.end}</option>`}).join('')}
                ...
                function renderBulkWeeks(root){const letters=cycleLetters(),field=document.getElementById('bulkWeekFieldFixed'),box=document.getElementById('bulkWeekChoicesFixed');if(!field||!box)return;const current=activeWeek(root),initial=letters.includes(current)?current:letters[0];bulkWeekSelection=new Set([initial]);field.hidden=letters.length===1;box.innerHTML='';letters.forEach(w=>{const b=document.createElement('button');b.type='button';b.className='bulkWeekChoice';b.textContent=w;const sync=()=>{const on=bulkWeekSelection.has(w);b.classList.toggle('active',on);b.setAttribute('aria-pressed',on?'true':'false')};sync();b.onclick=()=>{if(bulkWeekSelection.has(w)){if(bulkWeekSelection.size>1)bulkWeekSelection.delete(w)}else bulkWeekSelection.add(w);sync()};box.appendChild(b)})}
                function ensureModal(){let m=document.getElementById('bulkModalFixed');if(m)return m;m=document.createElement('div');m.id='bulkModalFixed';m.innerHTML='<form id="bulkSheetFixed"><h3>Ajouter plusieurs cours à une classe</h3><div class="bulkClass"><label>Classe / groupe</label><input id="bulkLabelFixed" required maxlength="80"></div><div id="bulkWeekFieldFixed" class="bulkWeekField"><label>Semaines concernées</label><div id="bulkWeekChoicesFixed" class="bulkWeekChoices"></div></div><div id="bulkRowsFixed" class="bulkRows"></div><button id="bulkMoreFixed" type="button" class="bulkMore">＋ Ajouter un autre jour / horaire</button><div class="bulkActions"><button type="button" id="bulkCancelFixed" class="btn">Annuler</button><button type="submit" class="btn primary">Ajouter tous les cours</button></div></form>';document.body.appendChild(m);document.getElementById('bulkCancelFixed').onclick=()=>m.classList.remove('show');m.onclick=e=>{if(e.target===m)m.classList.remove('show')};document.getElementById('bulkMoreFixed').onclick=()=>{const r=loadRoot(),w=activeWeek(r),rows=[...document.querySelectorAll('#bulkRowsFixed .bulkRow')],last=rows.at(-1),d=last?DAYS[(DAYS.indexOf(Number(last.querySelector('.bd').value))+1)%DAYS.length]:selDay();addRow(r,w,d,firstFree(r,w,d))};document.getElementById('bulkSheetFixed').onsubmit=e=>{e.preventDefault();let r=loadRoot();const chosen=[...bulkWeekSelection].filter(w=>cycleLetters().includes(w));if(!chosen.length)return;chosen.forEach(w=>ensureWeek(r,w));const label=document.getElementById('bulkLabelFixed').value.trim();if(!label)return;const ss=slots(r),seen=new Set(),adds=[],errs=[];document.querySelectorAll('#bulkRowsFixed .bulkRow').forEach(row=>{const d=Number(row.querySelector('.bd').value),n=Number(row.querySelector('.bs').value),room=row.querySelector('.br').value.trim(),slot=ss.find((item,index)=>(Number(item.n)||index+1)===n);if(!slot)return;const k=d+'|'+n;if(seen.has(k)){errs.push(DN[d]+' '+slot.start+' : doublon');return}seen.add(k);chosen.forEach(w=>{const list=r._weeks[w][String(d)].courses;const occupied=list.some(c=>mins(c.start)<mins(slot.end)&&mins(c.end)>mins(slot.start));if(occupied)errs.push('Semaine '+w+' · '+DN[d]+' '+slot.start+' : déjà occupé')});adds.push({d,start:slot.start,end:slot.end,label,room,slot:n})});if(errs.length){alert([...new Set(errs)].join(' • '));return}adds.forEach(x=>{const series=newSeries(),course={start:x.start,end:x.end,label:x.label,room:x.room,slot:x.slot,_series:series,weeks:[...chosen]};chosen.forEach(w=>r._weeks[w][String(x.d)].courses.push(course))});if(oneWeek())r=syncOne(r);m.classList.remove('show');saveRoot(r)};return m}
                function openBulk(){const r=loadRoot(),w=activeWeek(r),m=ensureModal();ensureWeek(r,w);document.getElementById('bulkLabelFixed').value='';document.getElementById('bulkRowsFixed').innerHTML='';renderBulkWeeks(r);const d=selDay();addRow(r,w,d,firstFree(r,w,d));m.classList.add('show')}

                function refresh(){ensureSingleState();ensureButton();ensureModeBar();wrapSave();bindWeekRepair();applySingleUi()}
                const preparers=window.__edtCoursePanelPreparers648||(window.__edtCoursePanelPreparers648=[]);
                if(!preparers.some(entry=>entry&&entry.id==='course-week-scope-679'))preparers.push({id:'course-week-scope-679',run:function(){
                  try{
                    const c=(typeof editing!=='undefined'&&editing!=null&&weeks[activeWeek]&&weeks[activeWeek][selected])?weeks[activeWeek][selected].courses[editing]||null:null;
                    if(typeof window.renderCourseWeekChoices==='function')window.renderCourseWeekChoices(c);
                  }catch(e){}
                }});
