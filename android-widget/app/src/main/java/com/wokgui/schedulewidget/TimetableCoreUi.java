        ...
        return """
            (function(){
              try {
                ...
                function classInk(color){
                  const m=/^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(String(color||''));if(!m)return '#17213a';
                  const channel=v=>{v/=255;return v<=.04045?v/12.92:Math.pow((v+.055)/1.055,2.4)},lum=(r,g,b)=>.2126*channel(r)+.7152*channel(g)+.0722*channel(b),contrast=(a,b)=>(Math.max(a,b)+.05)/(Math.min(a,b)+.05);
                  const bg=lum(parseInt(m[1],16),parseInt(m[2],16),parseInt(m[3],16)),dark=lum(0x17,0x21,0x3a);return contrast(bg,dark)>=contrast(bg,1)?'#17213a':'#ffffff'
                }
                function applyAppAppearance(){
                  ...
                  const mode=['stripe','fill'].includes(adv.appClassColorMode)?adv.appClassColorMode:(adv.appColorByClass?'stripe':'none');
                  document.querySelectorAll('.classTint,.classFill').forEach(el=>{el.classList.remove('classTint','classFill');el.style.removeProperty('--class-color');el.style.removeProperty('--class-ink')});
                  if(mode==='none')return;
                  const decorate=(el,label)=>{const color=classColor(label);el.classList.add(mode==='fill'?'classFill':'classTint');el.style.setProperty('--class-color',color);el.style.setProperty('--class-ink',classInk(color))};
                  document.querySelectorAll('.todayCourse').forEach(el=>{if(el.classList.contains('gap')||el.classList.contains('lunch'))return;const label=el.querySelector('.label');if(label)decorate(el,label.textContent)});
                  document.querySelectorAll('.wc.has').forEach(el=>{const label=el.querySelector('.cellLabel');if(label)decorate(el,label.textContent)});
                  document.querySelectorAll('.editCourse').forEach(el=>{const label=el.querySelector('.label');if(label)decorate(el,label.textContent)});
                }
        ...
        return """
            (function(){
              try {
                ...
                function applicationClassColorMode(){
                  try{const a=JSON.parse(AndroidSchedule.loadAdvancedSettings()||'{}');return ['stripe','fill'].includes(a.appClassColorMode)?a.appClassColorMode:(a.appColorByClass?'stripe':'none')}catch(e){return 'none'}
                }
                ...
                function readableCourseInk(color){
                  const m=/^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(String(color||''));if(!m)return '#17213a';
                  const channel=v=>{v/=255;return v<=.04045?v/12.92:Math.pow((v+.055)/1.055,2.4)},lum=(r,g,b)=>.2126*channel(r)+.7152*channel(g)+.0722*channel(b),contrast=(a,b)=>(Math.max(a,b)+.05)/(Math.min(a,b)+.05);
                  const bg=lum(parseInt(m[1],16),parseInt(m[2],16),parseInt(m[3],16)),dark=lum(0x17,0x21,0x3a);return contrast(bg,dark)>=contrast(bg,1)?'#17213a':'#ffffff'
                }
                function apply(el,id,gradient){
                  if(!el)return;const c=colorFor(id),mode=applicationClassColorMode();
                  el.style.removeProperty('background');el.style.removeProperty('box-shadow');el.style.removeProperty('border-left');
                  if(!c||mode==='none')return;
                  el.classList.remove('classTint','classFill');el.classList.add(mode==='stripe'?'classTint':'classFill');el.style.setProperty('--class-color',c);el.style.setProperty('--class-ink',readableCourseInk(c));
                }
            ...
            (function(){
              try{
                ...
                function style(){if(document.getElementById('bulkFixedStyle'))return;const s=document.createElement('style');s.id='bulkFixedStyle';s.textContent=`
                  #addBulkCourses{display:block!important;width:auto!important;max-width:100%;margin:7px auto 0;padding:9px 13px;border:1.5px solid var(--blue,#0877f9);border-radius:7px;background:#edf6ff;color:var(--blue,#0877f9);font-weight:800}
