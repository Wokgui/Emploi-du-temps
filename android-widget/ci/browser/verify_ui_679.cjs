...
(async()=>{
  ...
  if(!baseline){assert.deepEqual(errors,[]);assert.ok(report.navigation.active.every(x=>x.length===1));
    ...
    assert.deepEqual(report.settingsLayout.parents,{language:'languageSettings86',download:'languageSettings86',week:'weekTypeSettings86',weekColors:'colorSettings86',breaks:'breakSettings86',widget:'widgetSettings86',density:'textSettings86'});
    assert.ok(report.settingsLayout.summaries.every(x=>Math.abs(x.font-report.settingsLayout.summaries[0].font)<0.1));
    assert.ok(report.settingsLayout.selects.every(x=>x.h<=32&&x.font<=11.6));assert.ok(report.settingsLayout.advancedSeparators.length>0&&report.settingsLayout.advancedSeparators.every(x=>x>=1));
