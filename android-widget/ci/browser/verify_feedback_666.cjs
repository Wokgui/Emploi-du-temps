...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    window.__weekFrames666 = [];
    window.__sampleWeek666 = () => {
      const title = document.getElementById('weekTitleLetter').textContent.trim();
      const text = document.getElementById('weekGrid').innerText;
      window.__weekFrames666.push({ title, text, grids: document.querySelectorAll('.weekGrid').length, covers: document.querySelectorAll('.weekSwapCover669').length });
    };
  });
  for (let i = 0; i < 60; i++) {
    const letter = i % 2 ? 'A' : 'B';
    await page.locator('#weekTabs .weekTab[data-week="' + letter + '"]').evaluate(el => el.click());
    await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => { window.__sampleWeek666(); resolve(); })));
  }
  const weekFrames = await page.evaluate(() => window.__weekFrames666);
  assert.equal(weekFrames.length, 60);
  assert.ok(weekFrames.every(frame => frame.grids === 1 + frame.covers && frame.covers <= 1), JSON.stringify(weekFrames));
  ...
  console.log('feedback_666_standard_widget_direct=passed');
  console.log('feedback_666_condensed_preview=passed');
  console.log('feedback_666_atomic_week_switch=passed');
  console.log('feedback_666_atomic_edit_open=passed');
  console.log('feedback_666_days_off_order=passed');
  console.log('feedback_666_continuous_auto_density_all_widgets=passed');
  await context.close();
  await browser.close();
