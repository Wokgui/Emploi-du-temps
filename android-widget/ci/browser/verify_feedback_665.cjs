...
(async () => {
  ...
  const sliderReport = await page.evaluate(() => {
    ...
    slider.value = '2'; slider.dispatchEvent(new Event('input', { bubbles: true })); slider.dispatchEvent(new Event('change', { bubbles: true }));
  ...
  assert.deepEqual({ type: sliderReport.type, min: sliderReport.min, max: sliderReport.max, step: sliderReport.step }, { type: 'range', min: '0', max: '2', step: '1' });
  ...
  assert.equal(sliderReport.label, 'Confortable');
