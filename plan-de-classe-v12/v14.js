(function(){
  'use strict';
  const JOINED='joined', ISOLATED='isolated', MODEL='v14-one-seat';
  let longPressTimer=null,longPressSeat=null,suppressSeatClickUntil=0;
  let quickClassId=null,quickCols=2,quickTables=1;
  ...
  function normalizeClass(c=C()){
    ...
    if(c.modelVersion!==MODEL&&c.seatPerTable!==1){
      ...
      if(old.length>=n*2){
        ...
        const used=new Set(next.filter(Boolean)),overflow=[];
        for(let i=0;i<n;i++){
  ...
  function fitNames(){
    document.querySelectorAll('#room .seatname').forEach(n=>{
      ...
      const reserve=seat.querySelector('.seaticons')?15:5;
      const max=Math.max(18,seat.clientWidth-reserve-10);
  ...
  function onSeatPointerDown(e){
    ...
    const i=+e.currentTarget.dataset.i,sid=C().seats[i];if(!sid)return;
  ...
  function rowOffsets(c){const out=[];let n=0;c.rows.forEach((row,r)=>{out[r]=n;n+=rowTables(row)});return out}
  function makePlanRow(c,r,startIndex){
    const row=document.createElement('div');row.className='row';
    const groups=document.createElement('div');groups.className='tablegroups';let local=0;
    c.rows[r].forEach((count,ci)=>{
      ...
      for(let t=0;t<count;t++){
        const idx=startIndex+local++,sid=c.seats[idx],st=sid?studentById(sid,c):null;
    ...
    row.appendChild(groups);return row;
  }

  renderPlan=function(){
    const c=C();normalizeClass(c);const room=document.getElementById('room');room.innerHTML='';room.classList.remove('lmdrag');
    const stack=document.createElement('div');stack.className='rowsstack';const offsets=rowOffsets(c);
    // Vue depuis le fond vers le tableau : le rang 1 et le tableau sont en bas.
    for(let slot=c.rows.length;slot>=0;slot--){
      stack.appendChild(renderLandslot(c,slot));
      if(slot>0){const r=slot-1;stack.appendChild(makePlanRow(c,r,offsets[r]))}
    }
  ...
  function syncQuickFromClass(c){
    if(quickClassId===c.id)return;
    quickClassId=c.id;quickCols=Math.max(1,Math.min(6,c.rows[0]?.length||2));quickTables=Math.max(1,Math.min(8,c.rows[0]?.[0]||1));
    if(quickCols*quickTables>16)quickTables=Math.max(1,Math.floor(16/quickCols));
  }
  function renderQuickLayout(c,box){
    syncQuickFromClass(c);
    let quick=document.getElementById('quickLayoutV14');
    if(!quick){quick=document.createElement('section');quick.id='quickLayoutV14';quick.className='v14-quick';box.parentNode.insertBefore(quick,box)}
    quick.innerHTML='';
    const h=document.createElement('h3');h.textContent='Disposition rapide';
    const sub=document.createElement('p');sub.className='small';sub.textContent='Applique la même disposition à tous les rangs.';
    const controls=document.createElement('div');controls.className='v14-quick-controls';
    const makeControl=(label,get,set)=>{
      const wrap=document.createElement('div');wrap.className='v14-quick-control';
      const l=document.createElement('div');l.className='v14-quick-label';l.textContent=label;
      const step=document.createElement('div');step.className='v14-quick-step';
      const minus=makeStepButton('−',()=>{const v=get();if(v<=1)return;set(v-1);renderCfg()});
      const value=document.createElement('strong');value.textContent=get();
      const plus=makeStepButton('＋',()=>{const v=get();if(v>=8)return;set(v+1);if(quickCols*quickTables>16){if(label.startsWith('Colonnes'))quickTables=Math.max(1,Math.floor(16/quickCols));else quickCols=Math.max(1,Math.floor(16/quickTables))}renderCfg()});
      step.append(minus,value,plus);wrap.append(l,step);return wrap;
    };
    controls.append(
      makeControl('Colonnes par rang',()=>quickCols,v=>quickCols=Math.max(1,Math.min(6,v))),
      makeControl('Tables par colonne',()=>quickTables,v=>quickTables=Math.max(1,Math.min(8,v)))
    );
    const summary=document.createElement('div');summary.className='v14-quick-summary';summary.textContent=`${quickCols} colonne${quickCols>1?'s':''} × ${quickTables} table${quickTables>1?'s':''} par colonne`;
    const apply=document.createElement('button');apply.type='button';apply.className='pb v14-quick-apply';apply.textContent='Appliquer à tous les rangs';apply.onclick=()=>{
      if(quickCols*quickTables>16){toast('16 tables maximum par rang');return}
      mutate(()=>{
        c.rows=c.rows.map((_,r)=>{
          const oldModes=c.rowModes[r]||[];
          c.rowModes[r]=Array.from({length:quickCols},(_,i)=>oldModes[i]||JOINED);
          return Array(quickCols).fill(quickTables);
        });
      });
    };
    quick.append(h,sub,controls,summary,apply);
  }

  renderCfg=function(){
    const c=C();normalizeClass(c);document.getElementById('rowN').textContent=c.rows.length;
    const box=document.getElementById('rowCfg');box.innerHTML='';renderQuickLayout(c,box);
    let note=document.getElementById('cfgExplainV14');if(!note){note=document.createElement('p');note.id='cfgExplainV14';note.className='small cfghelp';box.parentNode.insertBefore(note,box)}
    note.textContent='Rangs : du tableau vers le fond. Colonnes : groupes de tables de gauche à droite dans un même rang.';
    c.rows.forEach((cols,r)=>{
      const card=document.createElement('section');card.className='rowcfg v14-rowcfg';
      const head=document.createElement('div');head.className='v14-rowhead';
      const title=document.createElement('b');title.textContent=rowTitle(r,c.rows.length);
      const lab=document.createElement('div');lab.className='v14-head-label';lab.textContent='Colonnes';
      const colCount=document.createElement('div');colCount.className='v14-inline-step';const val=document.createElement('span');val.className='v14-count';val.textContent=cols.length;
      colCount.append(makeStepButton('−',()=>{if(cols.length<=1)return;mutate(()=>{c.rows[r].pop();c.rowModes[r].pop()})}),val,makeStepButton('＋',()=>{if(cols.length>=6||rowTables(cols)>=16)return;mutate(()=>{c.rows[r].push(1);c.rowModes[r].push(JOINED)})}));
      head.append(title,lab,colCount);card.appendChild(head);
      const list=document.createElement('div');list.className='v14-cols';
      cols.forEach((tables,ci)=>{
        const col=document.createElement('div');col.className='v14-col';
        const ch=document.createElement('div');ch.className='v14-colhead';const cn=document.createElement('strong');cn.textContent=colTitle(ci,cols.length);const sum=document.createElement('span');sum.textContent=tables+' table'+(tables>1?'s':'');ch.append(cn,sum);
        const modes=document.createElement('div');modes.className='v14-modes';
        ...
        const count=document.createElement('div');count.className='v14-tablecount';const ctl=document.createElement('span');ctl.textContent='Nombre de tables';const step=document.createElement('div');step.className='step';const sv=document.createElement('span');sv.textContent=tables;
  ...
  newClass=async function(){const name=(await ask('Nom de la classe','Nouvelle classe'))?.trim();if(!name)return;const rows=INIT_ROWS.map(r=>[...r]),c={id:uid(),name,rows,students:[],seats:new Array(rows.reduce((a,r)=>a+rowTables(r),0)).fill(null),rowModes:rows.map(r=>r.map(()=>JOINED)),seatPerTable:1,modelVersion:MODEL,landmarks:{tableau:{visible:true,slot:0,align:'center'},bureau:{visible:true,slot:0,align:'right'}}};snap();S.classes.push(c);S.cur=c.id;quickClassId=null;save();render()};
  duplicateClass=function(){const src=C();normalizeClass(src);const c=JSON.parse(JSON.stringify(src));c.id=uid();c.name=src.name+' copie';const remap=new Map();c.students.forEach(s=>{const old=s.id;s.id=uid();remap.set(old,s.id)});c.seats=c.seats.map(id=>remap.get(id)||null);snap();S.classes.push(c);S.cur=c.id;quickClassId=null;save();render()};
  ...
  const sel=document.getElementById('classSel');if(sel){const old=sel.onchange;sel.onchange=e=>{quickClassId=null;if(old)old.call(sel,e)}}
  document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>showView(b.dataset.v));

  const planIcon=document.querySelector('.nav[data-v="plan"] .navico');
  if(planIcon){planIcon.innerHTML='<svg viewBox="0 0 28 28" aria-hidden="true"><rect x="3.2" y="3.5" width="21.6" height="21" rx="4"/><rect x="6.4" y="7" width="6" height="4.2" rx="1.2"/><rect x="15.6" y="7" width="6" height="4.2" rx="1.2"/><rect x="8.8" y="14" width="10.4" height="4.2" rx="1.2"/><path d="M7 22h14"/></svg>';planIcon.classList.add('v14-plan-icon')}

  render();
  window.__planV14={version:'14.0',settingsButtons:()=>document.querySelectorAll('#v-config button').length,seats:()=>document.querySelectorAll('#room .seat').length};
})();
